...
try {
  ...
  const create = (name, status = 'WAITING_DISTRIBUTION') => prisma.registration.create({ data: { title: `ADMIN RBAC ${name}`, registration_no: `ADMIN-RBAC-${name}`, publisher_id: publisher.id, service_type_id: service.id, status, fee_sla_snapshot: { sla_initial_days: 2, sla_revision_days: 2, sla_dummy_days: 2 } } });
  ...
  const deny = (path, method = 'POST', body = {}) => call(path, tokens.ADMIN, method, body, 403);
  ...
  const list = await call('/registrations?search=ADMIN%20RBAC&queue_only=true', tokens.ADMIN);
  assert.equal(list.pagination.total, 2);
  await call(`/registrations/${pending.id}`, tokens.ADMIN);
  await call(`/registrations/${pending.id}/timeline`, tokens.ADMIN);
  await call('/reports/verification-performance', tokens.ADMIN);
  await call(`/distribution-teams/${team.id}/workload`, tokens.ADMIN);
  ...
  for (const role of ['ADMIN', 'SUPERADMIN']) {
    ...
    await prisma.paymentRecord.create({ data: { registration_id: reg.id, billing_no: `ADMIN-BILL-${role}`, amount: 100, status: 'VERIFIED' } });
    ...
    assert.equal((await prisma.auditLog.findFirst({ where: { subject_id: assignment.id, action: 'CREATE_ASSIGNMENT' } })).actor_id, role === 'ADMIN' ? internalAdmin.id : superadmin.id);
    await call(`/registrations/${reg.id}/assignments`, tokens[role], 'POST', data, 409);
  }
  const unpaid = await create('unpaid');
  await call(`/registrations/${unpaid.id}/assignments`, tokens.ADMIN, 'POST', data, 409);
  const invalidTeam = await create('invalid-team');
  await prisma.paymentRecord.create({ data: { registration_id: invalidTeam.id, billing_no: 'ADMIN-BILL-invalid', amount: 100, status: 'VERIFIED' } });
  await call(`/registrations/${invalidTeam.id}/assignments`, tokens.ADMIN, 'POST', { ...data, assignee_ids: [verifier.id] }, 400);
  await call(`/registrations/${invalidTeam.id}/assignments`, tokens.ADMIN, 'POST', { ...data, team_id: 'missing-team' }, 409);
  const competing = await create('competing');
  await prisma.paymentRecord.create({ data: { registration_id: competing.id, billing_no: 'ADMIN-BILL-competing', amount: 100, status: 'VERIFIED' } });
  const attempts = await Promise.all([tokens.ADMIN, tokens.SUPERADMIN].map(token => fetch(`${base}/registrations/${competing.id}/assignments`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify(data) }).then(res => res.status)));
  ...
  console.log(`ADMIN RBAC integration passed: ${assertions} HTTP checks plus state, audit, concurrency and migration assertions.`);
} finally {
