...
test('separate admin guard only permits explicit ADMIN or SUPERADMIN membership', () => {
  for (const role of ['ADMIN', 'SUPERADMIN']) assert.equal(check(requireAdminInternal, { roles: [role] }).next, true);
  assert.equal(check(requireAdminInternal, { role: 'ADMIN' }).status, 403);
...
test('existing Kepala route guard rejects pure ADMIN and preserves Kepala access', () => {
  const guard = authorize('KEPALA_LPMQ');
  assert.equal(check(guard, { roles: ['ADMIN'] }).status, 403);
  assert.equal(check(guard, { roles: ['ADMIN', 'PENTASHIH'] }).status, 403);
  assert.equal(check(guard, { roles: ['KEPALA_LPMQ'] }).next, true);
  assert.equal(check(guard, { roles: ['ADMIN', 'KEPALA_LPMQ'] }).next, true);
