...
| Aksi | HELPER_ADMIN | SUPERADMIN | DISTRIBUTOR |
| --- | --- | --- | --- |
...
| Aksi penerbit, pemeriksaan verifikasi, sidang, pengesahan | Tidak melalui HELPER_ADMIN | Guard existing | Sesuai role existing |
...
yang memakai `authorize('HELPER_ADMIN', 'SUPERADMIN')`; endpoint assignment menggunakan
...
`HELPER_ADMIN` (Staf TU / Layanan) dan `SUPERADMIN`. `KEPALA_LPMQ` memiliki hak akses view-only dan ditolak
...
Pure HELPER_ADMIN mendapat 403. Multi-role bersifat aditif: HELPER_ADMIN + KEPALA_LPMQ tetap
mempunyai kewenangan dari KEPALA_LPMQ, bukan dari HELPER_ADMIN. Pembatasan service
...
menambahkan HELPER_ADMIN jika belum ada. ID role serta membership existing dipertahankan.
...
Skrip menambah membership HELPER_ADMIN, tidak menghapus role lain; idempotent dan audited.
