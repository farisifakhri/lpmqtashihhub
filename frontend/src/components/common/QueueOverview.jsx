  ...
  return (
    <section aria-label="Ringkasan antrean" className="grid grid-cols-1 sm:grid-cols-3 gap-3">
      <div className="rounded-xl border border-slate-200 bg-white p-4 flex items-center gap-3">
        <span className="rounded-lg bg-emerald-50 p-2.5 text-emerald-700"><Inbox className="h-5 w-5" /></span>
        <div><p className="text-xs text-slate-500">Dalam filter ini</p><p className="text-lg font-bold text-slate-900">{loading ? '…' : total} <span className="text-xs font-medium">naskah</span></p></div>
      </div>
      <div className="rounded-xl border border-emerald-200 bg-emerald-50/60 p-4 flex items-center gap-3">
        <ArrowDownWideNarrow className="h-5 w-5 shrink-0 text-emerald-700" />
        <div><p className="text-sm font-bold text-emerald-950">{fifo ? 'Tertua lebih dahulu · FIFO' : 'Riwayat terbaru dahulu'}</p><p className="text-xs text-emerald-800 mt-1">{fifo ? 'Urutan berdasarkan waktu masuk tahapan.' : 'Pekerjaan selesai dipisahkan dari antrean aktif.'}</p></div>
      </div>
      <div className="rounded-xl border border-slate-200 bg-white p-4 flex items-center gap-3">
...
export function QueuePagination({ pagination, loading, onPageChange, label = 'Navigasi halaman antrean' }) {
  ...
  return <nav aria-label={label} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white p-4 text-xs text-slate-600">
    ...
    <div className="flex gap-2">
      ...
      <button type="button" disabled={loading || pagination.page >= totalPages} onClick={() => onPageChange(pagination.page + 1)} className="rounded-lg border border-slate-300 px-3 py-2 font-semibold hover:bg-slate-50 disabled:opacity-40">Selanjutnya</button>
    </div>
  </nav>;
}
