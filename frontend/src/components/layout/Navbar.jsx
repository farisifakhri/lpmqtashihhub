...
export const Navbar = ({ sidebarOpen, onToggleSidebar }) => {
  ...
  const getRoleConfig = (userRole) => {
    switch (userRole) {
      case 'SUPERADMIN':
      case 'ADMIN':
        return { label: 'Administrator Sistem', color: 'bg-rose-50 text-rose-800 font-bold border-rose-200' };
      case 'VERIFIKATOR':
      case 'VERIFICATOR':
        return { label: 'Verifikator Dokumen', color: 'bg-sky-50 text-sky-800 font-bold border-sky-200' };
      case 'DISTRIBUTOR':
        return { label: 'Koordinator Distribusi', color: 'bg-amber-50 text-amber-800 font-bold border-amber-200' };
      case 'PENTASHIH':
      case 'TASHIH_MEMBER':
        return { label: 'Pentashih Naskah', color: 'bg-emerald-50 text-emerald-800 font-bold border-emerald-200' };
      case 'DOKUMENTATOR':
      case 'DOCUMENTATOR':
        return { label: 'Administrator Dokumen', color: 'bg-indigo-50 text-indigo-800 font-bold border-indigo-200' };
      case 'KEPALA_LPMQ':
      case 'HEAD_OF_LPMQ':
        return { label: 'Kepala LPMQ', color: 'bg-gold-50 text-gold-800 font-bold border-gold-300' };
      case 'ADMIN_PENERBIT':
      case 'PUBLISHER':
        return { label: 'Penerbit Terverifikasi', color: 'bg-teal-50 text-teal-800 font-bold border-teal-200' };
      default:
        return { label: userRole || 'Petugas', color: 'bg-slate-100 text-slate-800 font-bold border-slate-200' };
    }
  };
  ...
  return (
    <header className="bg-white/95 backdrop-blur-md border-b border-neutral-200 sticky top-0 z-40 transition-colors">
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
            ...
            >
              ...
              <span className="hidden md:inline">Validasi Dokumen</span>
            </Link>
                ...
                {profileDropdownOpen && (
                  <div className="absolute right-0 mt-2 w-64 bg-white rounded-xl shadow-lg border border-neutral-200 py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-150">
                    ...
                    <div className="px-4 py-3 border-b border-neutral-100">
                      <p className="text-xs text-neutral-400 font-medium uppercase tracking-wider">Sesi Aktif</p>
                    ...
                    <div className="px-4 py-2.5 bg-neutral-50 border-b border-neutral-100 text-[11px] text-neutral-600 flex items-center gap-2">
                      ...
                      <span>Kontrol proses mengacu pada SOP v2.2</span>
                    </div>
