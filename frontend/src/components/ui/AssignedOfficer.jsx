...
export const AssignedOfficer = ({
  ...
  return (
    <div
      className={clsx(
        'rounded-xl border border-line bg-canvas/70 p-3.5 sm:p-4 text-xs space-y-2',
        className
      )}
    >
      <div className="flex items-center gap-2 text-ink-muted font-medium">
        <ShieldCheck className="w-4 h-4 text-brand-800" aria-hidden="true" />
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="space-y-0.5">
          ...
          <p className="text-ink-muted font-medium">{role}</p>
        </div>

        {notaNo && (
          <span className="font-mono text-[11px] bg-white border border-line px-2 py-0.5 rounded text-ink">
            {notaNo}
          </span>
        )}
      </div>
      ...
      {formattedDate && (
        <div className="flex items-center gap-1.5 text-ink-muted pt-1 border-t border-line/60">
