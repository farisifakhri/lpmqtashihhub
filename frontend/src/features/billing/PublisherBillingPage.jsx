...
export const PublisherBillingPage = () => {
    ...
    useEffect(() => {
        ...
        let active = true;
        paymentApi
            .getRegistrationPayment(preselectedRegId)
            .then((response) => {
                if (!active) return;
                const payment = response.data?.id
                    ? response.data
                    : response.data?.payment;
                if (payment?.status === "UNPAID") openConfirmModal(payment);
                else if (!payment)
                    setSuccessMessage(
                        "Tagihan untuk naskah ini belum tersedia. Tunggu penerbitan kode billing oleh petugas.",
                    );
                else
                    setSuccessMessage(
                        "Bukti pembayaran naskah ini sudah dikirim atau diverifikasi. Tidak perlu mengirim ulang.",
                    );
            })
            .catch((err) => {
                if (active)
                    setError(
                        err.message || "Tagihan naskah tidak dapat dimuat.",
                    );
            });
        return () => {
            active = false;
    ...
    const handleConfirmSubmit = async (e) => {
        ...
        if (!ntpn.trim()) {
            setModalError(
                "Nomor Transaksi Penerimaan Negara (NTPN) / Ref Transfer wajib diisi.",
            );
            return;
        }
        ...
        try {
            ...
            setSuccessMessage(
                "Konfirmasi pembayaran berhasil dikirimkan. Petugas LPMQ akan memverifikasi bukti Anda.",
            );
    ...
    const metrics = useMemo(() => {
        return {
            ...
            verifiedCount: payments.filter((p) => p.status === "VERIFIED")
                .length,
        };
    }, [payments]);
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div className="space-y-1.5">
                            ...
                            <p className="text-xs sm:text-sm text-emerald-100/80 max-w-2xl leading-relaxed">
                                Kelola kode billing resmi Kementerian Agama,
                                pantau masa berlaku tagihan 7 hari kalender, dan
                                lakukan konfirmasi setoran NTPN secara aman.
                            </p>
                        </div>
                        ...
                        <Button
                            ...
                            disabled={loading}
                            variant="outline"
                            className="text-xs text-emerald-900 border-emerald-500/50 hover:bg-emerald-800/60 hover:text-white shrink-0 self-start sm:self-center"
                        >
                            <RefreshCw
                                    ...
                                    <div className="p-3.5 bg-rose-50 border border-rose-200 rounded-xl flex items-start gap-2.5 text-xs text-rose-800 animate-fadeIn">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-rose-600 text-[11px] mt-1 italic">
                                                Silakan unggah kembali bukti
                                                setoran bank yang sah dan
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        {item.external_ref ? (
                                            <p className="font-mono font-bold text-slate-800">
                                                NTPN: {item.external_ref}
                                            </p>
                                        ) : (
                                            <p className="text-amber-700 font-medium italic">
                                                Belum ada konfirmasi NTPN
                                            </p>
                                        )}
                                        <p className="text-slate-500">
                                            Terbit:{" "}
                    ...
                    <div className="bg-white rounded-2xl max-w-lg w-full p-6 space-y-4 shadow-2xl border border-slate-200 animate-slideUp">
                        ...
                        <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-xl space-y-2 text-xs">
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                <span className="font-bold text-slate-800">
                                    {selectedPayment.registration?.title || "-"}
                                </span>
                            </div>
                        ...
                        >
                            <div>
                                ...
                                <span className="text-[11px] text-slate-400 mt-1 block">
                                    Nomor NTPN tercetak pada bukti setoran bank
                                    atau struk teller/ATM.
                                </span>
                            </div>

                            <div>
                                <label className="block text-xs font-bold text-slate-800 mb-1">
                                    Unggah Bukti Setor / Transfer{" "}
                                    <span className="text-rose-600">*</span>
                                </label>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-white p-4 rounded-xl border border-slate-200 space-y-1.5 shadow-2xs">
                        <p className="font-bold text-slate-800">
                            1. Salin Kode Billing
                        </p>
                        <p className="text-[11px] leading-relaxed text-slate-500">
                            Gunakan Nomor Billing yang tertera di atas. Pastikan
                            pembayaran dilakukan sebelum batas waktu SLA (7 hari
                            kalender).
                        </p>
                    </div>
                    ...
                    <div className="bg-white p-4 rounded-xl border border-slate-200 space-y-1.5 shadow-2xs">
                        <p className="font-bold text-slate-800">
                            2. Setor ke Bank Persepsi
                        </p>
                        <p className="text-[11px] leading-relaxed text-slate-500">
                            Bayar melalui Teller Bank, ATM, atau Mobile Banking
                            mitra SIMPONI (Bank Mandiri, BRI, BNI, BSI, BCA)
                            menggunakan menu Pembayaran Penerimaan Negara.
                        </p>
                    </div>

                    <div className="bg-white p-4 rounded-xl border border-slate-200 space-y-1.5 shadow-2xs">
                        <p className="font-bold text-slate-800">
                            3. Konfirmasi di Portal
                        </p>
                        <p className="text-[11px] leading-relaxed text-slate-500">
                            Masukkan Nomor Transaksi Penerimaan Negara (NTPN)
                            serta unggah foto/PDF bukti bayar agar verifikator
                            LPMQ dapat memverifikasi pelunasan.
                        </p>
                    </div>
                </div>
            </div>
