...
export function CoreTeamPage() {
  ...
  return <main className="max-w-6xl mx-auto p-6 space-y-6">
    <div>
      ...
      <p className="text-sm text-slate-600">Tim baru berlaku untuk pengajuan berikutnya. Tim yang sudah melekat pada pengajuan tetap sama.</p>
    </div>
    ...
    <section className="border rounded-xl p-4 space-y-3">
      ...
      <div className="flex gap-3 items-center">
        ...
        <button type="button" disabled={busy || !config || reason.trim().length < 5} onClick={() => save('/core-teams/next', { next_team_number: nextNumber, reason: reason.trim() })} className="bg-emerald-700 text-white rounded px-4 py-2 disabled:opacity-50">Simpan nomor</button>
      </div>
    </section>
    <section className="border rounded-xl p-4 space-y-3">
      <h2 className="font-semibold">Sembilan tim inti</h2>
      <p className="text-sm text-slate-600">Cocokkan setiap nama pada daftar tim resmi dengan akun aktif. Menyimpan membuat versi roster baru.</p>
      ...
      <button type="button" disabled={busy || reason.trim().length < 5 || teams.some(team => fields.some(([field]) => !team[field]))} onClick={() => save('/core-teams/roster', { teams, reason: reason.trim() })} className="bg-emerald-700 text-white rounded px-4 py-2 disabled:opacity-50">Simpan versi roster</button>
    </section>
  </main>;
}
